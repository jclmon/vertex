...
public class PortfolioServiceImpl implements PortfolioService {
   ...
   * 
   * You access the event bus using vertx.eventBus()
   * 
   * @param action
   * @param amount
   * @param quote
  ...
  /**
   * 
   * Fill the evaluate method to retrieve the quotes service. 
   * 
   * You can retrieve Http services using HttpEndpoint.getClient. The name of the service is quotes.
   * 
   */
  @Override
  ...
  private void computeEvaluation(WebClient webClient, Handler<AsyncResult<Double>> resultHandler) {
	 //List of futures Different evaluations that would receive the different evaluations (one per company)
    ...
    //CompositeFuture.all calls its assigned handler when all the given Futures are assigned
    CompositeFuture.all(results).setHandler(
        ar -> {
       	  //we knows all the futures has received a value, and so we can compute the sum
